...
1. **No model ever abstained**, and 199 of 200 responses showed no hedging either. Of the 40 wrong answers in the dataset, 39 were asserted with full confidence; the single `hedged_wrong` (qwen, on a version-trap question) is the only time any model signaled uncertainty while being wrong. In this domain, "wrong" and "confidently wrong" were effectively the same thing.
